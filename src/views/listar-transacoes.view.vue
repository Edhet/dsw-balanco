...
const saldo = computed(() => {
    return listaTransacoes.reduce((acc, currentValue) => {
        if (currentValue.tipo === "DEBITO") { return acc -= currentValue.valor }
...
function deletarTransacao(descricao: string) {
    ...
    setTimeout(() => {
        if (itemsPagina.value.length == 0 && listaTransacoes.length != 0)
...
<template>
    <section>
        ...
        <div class="lista-footer">
            ...
            <button :disabled="isButtonDisabled || pagina * 5 - 5 > pagina"
                @click="mudarPagina('prox')">Próximo</button>
        </div>
    </section>
</template>
